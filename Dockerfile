...
RUN sed -i 's/33:33/99:100/g' /etc/passwd \
    ...
    && sed -i 's/pm.max_children = 5/pm.max_children = 150/g' /usr/local/etc/php-fpm.d/www.conf \
    && sed -i 's/pm.max_spare_servers = 3/pm.max_spare_servers = 130/g' /usr/local/etc/php-fpm.d/www.conf \
